...
- [~] **Этап 2 — Перенос LLM-стенда в поды** — **отменён пользователем 2026-09-12**: переносить ничего не нужно, k8s — площадка для новых экспериментов. Манифесты стенда остаются в `stand/` как референс.
- [ ] **Этап 3 — Опционально (backlog):** ingress/доступ из LAN, update/backup-скрипты, расширенный health_check — по явной потребности.
...
- **Статус:** задача выполнена (этапы 0–1); k8s-площадка готова к экспериментам
- **Активный запуск:** — (при новых задачах создавать workspace/runs/)
